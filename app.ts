...
// Module-level client (anon key, no user context) — used only for the
// connectivity check at /api/supabase/status. Every real data route uses
// supabaseForRequest() instead so Postgres RLS applies as the calling user.
const supabase = process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY
  ? createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY)
  : null;

/**
 * Builds the Express app with all API routes, shared between:
 * - server.ts (local dev / traditional Node hosting, adds Vite middleware + app.listen)
 * - api/index.ts (Vercel Serverless Function, no listen/Vite middleware needed)
 */
export function createApp(): Express {
  const app = express();
  app.use(express.json());

  // Per-request authenticated client — forwards the caller's own Supabase
  // session bearer token so every query runs as that user and Postgres RLS
  // (authenticated_full_access policies) applies. Credentials never reach the browser.
  ...
  // ---------------------------------------------------------------------
  // Auth — Supabase email/password (the app's own login). Independent from
  // the Firebase Google OAuth used client-side purely for Drive/Calendar access.
  // ---------------------------------------------------------------------
  ...
  app.delete('/api/institutions/:id', async (req, res) => {
    ...
    if (error) {
      // FK RESTRICT on demands/deals fires a Postgres 23503 — surface it as a
      // clear, actionable message instead of a raw constraint error string.
      if (error.code === '23503') {
  ...
  app.post('/api/deals', async (req, res) => {
    ...
    // New card goes to the bottom of its stage column.
  ...
  // Batch reorder — called once per drag-and-drop drop with the full final
  // ordering of the destination column (including the moved card, even if
  // it came from another column). Re-numbering the whole column avoids
  // fractional-index drift and is cheap at boutique-firm data volumes.
